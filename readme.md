...
```bash  
# 基础编译（不使用图像识别）
g++ main.cpp -o app -lUser32 -lOle32 -lgdiplus

# 使用图像识别功能（需安装OpenCV）
g++ main.cpp -o app -lUser32 -lOle32 -lgdiplus -lopencv_core -lopencv_highgui -lopencv_imgproc -lopencv_imgcodecs
```  
...
| WriteLog   | const std::string& str, const std::string& fileName, bool utf8=true | bool | 写日志（追加模式）           |
...
| 函数          | 参数                                      | 返回值                  | 说明                            |
|-------------|-----------------------------------------|-----------------------|-------------------------------|
| 构造函数       | const string &file_path, const string &file_mode, bool useGBK=true, bool auto_close=true | - | 打开文件，"r", "w", "a", "rb", "wb", "ab" |
| open        | const string &file_path, const string &file_mode, bool useGBK=true, bool auto_close=true | void | 打开文件                        |
...
// 读取JSON
wkp::Open jsonFile;
jsonFile.open("data.json", "r", true);
auto dict = jsonFile.readJson();
cout << "name: " << dict["name"] << endl;

// 二进制操作
vector<unsigned char> binData = {0x48, 0x65, 0x6C, 0x6C, 0x6F};
opt.open("data.bin", "wb");
opt.writeBinary(binData);
opt.close();

// 字符串转二进制写入
opt.open("encrypted.bin", "wb");
opt.writeStringToBinary("敏感数据");
opt.close();
```  
...
| 函数   | 参数     | 返回值   | 说明 |
|------|--------|--------|------|
| setAutoStartUp | const std::string &appName, bool useGBK=true | bool | 将当前可执行文件列入开机自启注册表 |
| setAutoStartUp | const std::string &exePath, const std::string &appName, bool useGBK=true | bool | 将指定文件路径的可执行文件列入开机自启注册表 |
...
// 指定程序开机自启
bool b = wkp::setAutoStartUp("C:\\Programs\\myapp.exe", "myapp2");
```  

#### `wkp::disableAutoStart` - 禁用开机自启
| 函数   | 参数     | 返回值   | 说明 |
|------|--------|--------|------|
| disableAutoStart | const string& appName, bool useGBK=true | bool | 禁止并删除开机自启 |
...
**返回值**: string - 格式如 "2026-03-06 15:30:45"
...
1. **管理员权限**：部分操作（如进程终止、开机自启、驱动级模拟、设置图标）需以管理员身份运行程序
...
### 2026/3/12 更新
1. 新增 `优化了println与format的bug，增加了{0},{1}的格式化输入支持`
