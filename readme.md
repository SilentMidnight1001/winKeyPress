...
mouse.MouseSideKeyPress("back", 1, 0.1);     // 按下后退键  
```  
...
KeyBoardEvent k;  
thread keyThread([&](){  
    k.ListenOneKeyEvent(); // 默认实时模式  
    // 或 k.ListenOneKeyEvent(false); // 非实时模式  
});  
keyThread.detach();  
...
```cpp  
ImageEvent imge;
imge.CaptureScreen("screenshot", "../images/");
```  
...
**返回值**: string - 格式如 "2026-02-17 15:30:45"
...
1. **管理员权限**：部分操作（如进程终止、开机自启）需以管理员身份运行程序
