...
```cpp
// 此函数为重载函数，可选参数为是否实时监听按键 //
// 默认为取消实时监听，仅当按键状态发生变化时触发，比如按钮按下长按后抬起 //
ListenOneKeyEvent();
ListenOneKeyEvent(bool real_time);

// 使用方法 //
// 当使用线程调用监听函数以后: ListenOneKeyEvent() //
KeyBoardEvent k;
int main()
{
    thread T([(){ListenOneKeyEvent();}]);
    while(true)
    {
        long KeyCode = k.GetKeyCode();
        switch (KeyCode)
        {
            case KEY_ESC_DOWN:
                ...to do something...
...
#### 9. `GetImagePosition()` - 获取图像在屏幕中的位置
```cpp
int main()
{
    ImagePosition  img = GetImagePosition("img.png", 0.85);
    cout<<img.x<<" "<<img.y<<endl;
    // if img not finde img.result=false
    if (img.result)
    {
        MouseEvent m;
        m.MouseClick(img.x, img.y, 1, "left");
    }
    return 0;
}
```
...
*2025/4/27更新*
1. **增加新功能**：单按键监听任务，使用ListenOneKeyEvent()执行键盘事件监听，并且另外开一个线程进行获取键盘事件码做出对应任务
2. **函数修改**：关于对PressHotKey()函数的修改，现在按下快捷键的数量为任意参数
*2025/5/5更新*
1. ** 增加新功能**：增加对图像识别的支持，使用GetImagePosition()函数即可获取图像在屏幕中的位置，可以使用MouseClick进行点击
