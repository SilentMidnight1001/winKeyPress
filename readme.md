...
## 开机自启/禁用开机自启
#### `wkp::setAutoStartUp` /`wkp::disableAutoStart`-
| 函数   | 参数   | 函数类型         | 说明                          |
|------|------|--------------|-----------------------------|
| setAutoStartUp | bool | string | 重载函数，将当前可执行文件列入开机自启注册表      |
| setAutoStartUp | bool | string /string | 重载函数，将指定文件路径的可执行文件列入开机自启注册表 |     |
| disableAutoStart | bool | string       | 禁止开机自启                      |
```cpp  
int main()
{
    bool a = setAutoStartUp("app1")
    bool b = setAutoStartUp("./eee.exe", "app2")
    bool c = disableAutoStart("app1")
    bool c = disableAutoStart("app2")
}
```
...
| 函数   | 参数   | 函数类型   | 说明             |
|------|------|--------|----------------|
| callPowerShell | bool | string | 执行powershell命令 |
```cpp  
int main()
{
    string path = "111+555";
    bool c = wkp::callPowerShell(path);
    cout<<c<<endl;
}
```
...
*2025/8/22更新*
1. 新增函数 `CaptureAreaScreen` 实现区域截图
2. 新增函数 `wkp::setAutoStartUp` 实现开机自启(用户级)
3. 新增函数 `wkp::disableAutoStart` 禁用并删除开机自启(用户级)
4. 新增函数 `wkp::listDir` 可以用来代替GetListDir,一个使用win api lisDir借助c++17的filesystem
5. 更新功能，实现原缩放DPI的坐标支持
6. 更新功能 `wkp::callPowerShell` 执行PowerShell命令

* 使用提醒: *
1. 注意!!大多数函数需要使用StringCode做编码转化，尽量使用utf8编码写项目，因为Windows原生api大多数gbk，我无法对此问题进行解决，一直寻找问题，如果出现莫名错误，尝试转为GBK编码康康
