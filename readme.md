...
int main()
{
    // 受限于filesystem仅在utf8编码才不会报错 //
    wkp::open file("D:\\QtCheakPySource\\pyinstaller反编译/pyinstaller反编译.exe", "rb");
    if (!file.is_open())
    {
        return -1;
    }
    auto date = file.readBinary();

    wkp::open file2("./test.exe", "wb");
    if (!file2.is_open())
    {
        return -1;
    }
    file2.writeBinary(date);
