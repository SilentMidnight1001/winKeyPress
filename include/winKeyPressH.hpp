...
// 存放快捷键虚键码的容器 //
vector<KeyCode>key_code;


// 快捷键的虚键码 //
vector<int>fourKeyCode;  // 四个键位的键值
vector<int> threeKeyCode;  // 三个键位的键值
vector<int> twoKeyCode;  // 两个键位的键值
vector<int> oneKeyCode;  // 一个键位的键值
...
vector<std::function<void()>>oneKeyFunc;

// 快捷键计数器 //
int fourKeyIndex = 0;  // 四个键位的索引
int threeKeyIndex = 0;  // 三个键位的索引
int twoKeyIndex = 0;  // 两个键位的索引
int oneKeyIndex = 0;  // 一个键位的索引
